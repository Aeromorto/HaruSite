(() => {
  ...
  try {
    if (localStorage.getItem(KEY) === null) {
      let legacy = [];
      try { legacy = normalize(JSON.parse(localStorage.getItem('haru-cart') || '[]')); } catch (_) { /* corrupt legacy data */ }
      localStorage.setItem(KEY, JSON.stringify(legacy));
    }
  } catch (_) { storageAvailable = false; }
  // Cleanup remains independent of quota failures during cart migration.
  ['haru-checkout', 'haru-order-draft'].forEach((key) => {
    try { localStorage.removeItem(key); } catch (_) { /* storage denied */ }
  });
  try { if (localStorage.getItem(KEY) !== null) localStorage.removeItem('haru-cart'); } catch (_) { /* storage denied */ }
  ['haru-cart', 'haru-checkout'].forEach((key) => {
    try { document.cookie = key + '=; path=/; max-age=0; SameSite=Lax'; } catch (_) { /* cookies disabled */ }
  });
  load();
  const persist = () => {
